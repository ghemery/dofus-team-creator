...
import { ROLE_LABELS, RoleType, getTier } from '../types';
...
export function SuggestionsPage() {
  ...
  const handleSave = (s: TeamSuggestion) => {
    ...
    addTeam(s.roles, s.autoScore, s.label);
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '1.5rem 1rem' }}>
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        <p style={{ color: '#8b949e', margin: '0.25rem 0 0', fontSize: '0.9rem' }}>
          Compositions générées automatiquement selon les statistiques de chaque classe.
        </p>
      </div>
      ...
      {suggestions.length === 0 && (
        ...
        >
          Aucune suggestion disponible. Vérifiez que les classes sont chargées.
        </div>
      )}

      <div style={{ marginTop: '1.5rem', textAlign: 'center' }}>
        <button className="dofus-btn-outline" onClick={() => navigate('/tierlist')}>
          🏆 Voir le classement
        </button>
      </div>
    </div>
